...
fn get_httpd_pid() -> Option<libc::pid_t> {
	// This file generated by Apache contains its PID
...
pub fn up() {
	...
	// Delete the old pid file created by Apache before trying to spawn a new one.
		...
		Ok(status) => {
			if status.success() {
				*guard = get_httpd_pid();
				...
				glib::g_warning!(
					LOG_DOMAIN,
					"Error spawning httpd: spawning process terminated by a signal"
				);
			}
		}
		Err(e) => {
